<template>
  <div class="table-wrapper">
    <div class="actions">
      <button @click="getData">
        Обновить
      </button>
      <button
        v-if="isAdmin"
        @click="onAddUser"
      >
        Добавить Пользователя
      </button>
    </div>
    <table class="fl-table">
      <thead>
        <tr>
          <th>Имя</th>
          <th>Фамилия</th>
          <th>Роль</th>
          <th v-if="isAdmin">
            Действия
          </th>
        </tr>
      </thead>


      <tbody>
        <tr
          v-for="user in users"
          :key="user.id"
        >
          <td>{{ user.name }}</td>
          <td>{{ user.surname }}</td>
          <td>{{ user.role }}</td>
          <td>
            <span
              v-if="isAdmin && userId !== user.id"
              class="delete"
              @click="deleteUser(user.id)"
            >Удалить</span>
            <span
              class="update"
              @click="updateUser(user)"
            >Обновить</span>
          </td>
        </tr>
      </tbody>
    </table>
  </div>
  <Modal
    v-if="showModal"
    type="users"
    :title="title"
    @close="showModal = false"
  >
    <Form :form="form" />
    <div
      v-if="type !== 'add'"
      class="dop-actions"
    >
      <input
        v-model="isPasswordChange"
        type="checkbox"
      >
      <span>Изменить пароль?</span>
      <div
        v-if="isPasswordChange"
        class="changePassword"
      >
        <InputUi
          v-model="passwordValue"
          placeholder="Новый пароль"
        />
      </div>
    </div>
    <button
      class="button"
      :disabled="disabled"
      @click="handler"
    >
      {{ title }}
    </button>
  </Modal>
</template>

<script setup lang="ts">
import {computed, onMounted, ref} from "vue"
import axios from "axios"
import Modal from "@/components/Modal.vue"
import Form from "@/components/Form.vue"
import {useNotification} from "@kyvg/vue3-notification"
import InputUi from "@/components/inputUi.vue"

const DEFAULT_FORM = [{
  type: 'inputUi',
  placeholder: 'Имя',
  field: 'name',
  value: '',
},
  {
    type: 'inputUi',
    placeholder: 'Фамилия',
    field: 'surname',
    value: '',
  },
  {
    type: 'inputUi',
    placeholder: 'Логин для входа в систему',
    field: 'username',
    value: '',
  },
  {
    type: 'inputUi',
    placeholder: 'Пароль',
    field: 'password',
    value: '',
  },]
const showModal = ref(false)
const users = ref([])
const type = ref('add')
const form = ref(DEFAULT_FORM)

const userId = Number(localStorage.getItem('userId'))
const {notify} = useNotification()
const token = ref(localStorage.getItem('token'))
const title = ref('Добавить пользователя')
const currentUserId = ref(null)
const passwordValue = ref('')


const isAdmin = computed(() => {
  return localStorage.getItem('role') === "ADMIN"
})

const isPasswordChange = ref(false)

onMounted(async () => {
  await getData()
})

const disabled = computed(() => {
  let error = false
  for (const formItem of form.value) {
    if (!formItem.value) error = true
  }
  return error
})

const handler = () => {
  if (type.value === 'add') {
    addUser()
  } else {
    onUpdateUser()
  }
}


const onAddUser = () => {
  title.value = 'Добавить пользователя'
  type.value = 'add'
  form.value = DEFAULT_FORM
  showModal.value = true
}

const onUpdateUser = async () => {
  const params = {}
  form.value.forEach((el) => {
    params[el.field] = el.value
  })
  params.id = currentUserId.value
  if(isPasswordChange.value){
    params.password = passwordValue.value
  }
  try {
    const data = await axios.put('http://localhost:8080/api/user', params, {
      headers: {
        Authorization: `Bearer ${token.value}`,
      }
    })
    if (data.status === 200) {
      notify({text: 'Пользователь успешно обновлен'})
    } else {
      notify({text: 'Произошла ошибка'})
    }
    showModal.value = false
    await getData()
  } catch (e) {
    notify({text: e.message})
  }
}


const updateUser = (user) => {
  form.value = [
    {
      type: 'inputUi',
      placeholder: 'Имя',
      field: 'name',
      value: user.name,
    },
    {
      type: 'inputUi',
      placeholder: 'Фамилия',
      field: 'surname',
      value: user.surname,
    },
  ]
  type.value = 'update'
  title.value = 'Обновить пользователя'
  currentUserId.value = user.id
  showModal.value = true
}

const addUser = async () => {
  const params = {}
  form.value.forEach((el) => {
    params[el.field] = el.value
  })
  try {
    const data = await axios.post('http://localhost:8080/api/user', params, {
      headers: {
        Authorization: `Bearer ${token.value}`,
      }
    })
    if (data.status === 200) {
      notify({text: 'Пользователь успешно добавлена'})
    } else {
      notify({text: 'Произошла ошибка'})
    }
    showModal.value = false
    await getData()
  } catch (e) {
    notify({text: e.message})
  }
}

const deleteUser = async (id) => {
  try {
    const data = await axios.delete(`http://localhost:8080/api/user/${id}`, {
      headers: {
        Authorization: `Bearer ${token.value}`,
      }
    })
    if (data.status === 200) {
      notify({text: "Пользователь был успешно удален"})
    } else {
      notify({text: "Произошла ошибка"})
    }
    await getData()

  } catch (e) {
    notify({text: e.message})
  }
}

const getData = async () => {
  try {
    const {data} = await axios.get('http://localhost:8080/api/user', {
      headers: {
        Authorization: `Bearer ${token.value}`,
      }
    })
    users.value = data
    if (data.status === 'error') {
      notify({
        text: data.message,
      })
    }

  } catch (e) {
    notify({
      text: 'Токен доступа просрочен',
    })
  }
}


</script>

<style scoped>
.delete {
  color: red;
  cursor: pointer;
  margin-right: 5px;
}

.actions {
  display: flex;
  align-items: flex-end;
  justify-content: flex-end;
  gap: 8px;
  margin-bottom: 10px;

  button {
    cursor: pointer;
    outline: none;
    border: 1px solid #4C489D;
    border-radius: 5px;
    padding: 10px;
    display: flex;
    align-items: center;
    background: none;

    &:hover {
      border: 1px solid #ffffff;
      background: #4C489D;
      color: #ffffff;
    }
  }
}

* {
  box-sizing: border-box;
  -webkit-box-sizing: border-box;
  -moz-box-sizing: border-box;
}

body {
  font-family: Helvetica;
  -webkit-font-smoothing: antialiased;
  background: rgba(71, 147, 227, 1);
}


/* Table Styles */

.table-wrapper {
  margin: 10px 70px 70px;
  box-shadow: 0px 35px 50px rgba(0, 0, 0, 0.2);
}

.fl-table {
  border-radius: 5px;
  font-size: 16px;
  font-weight: normal;
  border: none;
  border-collapse: collapse;
  width: 100%;
  max-width: 100%;
  white-space: nowrap;
  background-color: white;
}

.fl-table td, .fl-table th {
  text-align: center;
  padding: 8px;
}

.fl-table td {
  border-right: 1px solid #f8f8f8;
  font-size: 16px;
}

.fl-table thead th {
  color: #ffffff;
  background: #324960;
}


.fl-table thead th:nth-child(odd) {
  color: #ffffff;
  background: #324960;
}

.fl-table tr:nth-child(even) {
  background: #F8F8F8;
}

/* Responsive */

@media (max-width: 767px) {
  .fl-table {
    display: block;
    width: 100%;
  }

  .table-wrapper:before {
    content: "Scroll horizontally >";
    display: block;
    text-align: right;
    font-size: 16px;
    color: white;
    padding: 0 0 10px;
  }

  .fl-table thead, .fl-table tbody, .fl-table thead th {
    display: block;
  }

  .fl-table thead th:last-child {
    border-bottom: none;
  }

  .fl-table thead {
    float: left;
  }

  .fl-table tbody {
    width: auto;
    position: relative;
    overflow-x: auto;
  }

  .fl-table td, .fl-table th {
    padding: 20px .625em .625em .625em;
    height: 60px;
    vertical-align: middle;
    box-sizing: border-box;
    overflow-x: hidden;
    overflow-y: auto;
    width: 120px;
    font-size: 16px;
    text-overflow: ellipsis;
  }

  .fl-table thead th {
    text-align: left;
    border-bottom: 1px solid #f7f7f9;
  }

  .fl-table tbody tr {
    display: table-cell;
  }

  .fl-table tbody tr:nth-child(odd) {
    background: none;
  }

  .fl-table tr:nth-child(even) {
    background: transparent;
  }

  .fl-table tr td:nth-child(odd) {
    background: #F8F8F8;
    border-right: 1px solid #E6E4E4;
  }

  .fl-table tr td:nth-child(even) {
    border-right: 1px solid #E6E4E4;
  }

  .fl-table tbody td {
    display: block;
    text-align: center;
  }
}

.button {
  margin-top: 20px;
  padding: 10px;
  background: #324960;
  color: #ffffff;
  cursor: pointer;
}

.button:disabled {
  cursor: not-allowed;
}

.update {
  cursor: pointer;
}

.dop-actions{
  margin-top: 10px;
  input{
    margin-right: 8px;
  }
}


.changePassword{
  margin-top: 10px;
}
</style>